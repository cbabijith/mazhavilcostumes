...
- Keep the Order Items UI to Good, Damaged, Not Returned, Returning Now, Discount and a return action, with conditional damage details. Multi-unit outstanding rows have editable return counts. Limit damaged units to the count returning now. Show the settlement breakdown only when a discount or fee is present, and a separate Payment Due action whenever a balance remains. Keep all totals and payment limits synchronized with the same draft; typing must not persist changes. Do not add extra late-fee or return-notes inputs.
- Match current web return validation: unmarked rows do not show a partial warning but block submission; Good/Damaged requires a positive count; at least one unit must return. Send only rows returning units, using cumulative quantities. Skip already-returned rows in Mark All Good and submissions, retaining their saved damage. The current web backend also supports flagged orders with units still out; see README for the older local admin-service limitation.
