...
class _CategoryFormViewState extends ConsumerState<CategoryFormView> {
  ...
  void _onNameChanged() {
    if (!mounted) return;
    _updateSlugIfAutoGenerated();
  }

  void _updateSlugIfAutoGenerated() {
    if (!mounted) return;
    final oldAutoSlug = isEditing ? _generateCategorySlug(widget.category!.name, widget.category!.parentId) : '';
    final rawSlug = _generateSlug(_nameController.text);
    if (!isEditing || _slugController.text == oldAutoSlug || _slugController.text == rawSlug) {
      final newSlug = _generateCategorySlug(_nameController.text);
  ...
  String _generateCategorySlug(String name, [String? parentIdOverride]) {
    final rawSlug = _generateSlug(name);
    if (rawSlug.isEmpty) return '';

    final targetParentId = parentIdOverride ?? _parentId;
    if (targetParentId != null && mounted) {
      ...
      if (parent != null) {
        final parentSlug = parent.slug.isNotEmpty
            ? parent.slug
            : _generateSlug(parent.name);
        if (parentSlug.isNotEmpty) {
  ...
  Widget _buildParentSelector() {
    ...
    return categoriesAsync.when(
      loading: () => const SizedBox(height: 48, child: Center(child: Text('Loading categories...'))),
  ...
  Widget _buildReadOnlyParent() {
    final categoriesAsync = ref.watch(categoriesProvider);
    return categoriesAsync.when(
      loading: () => const SizedBox(height: 36, child: Center(child: Text('Loading parent...'))),
