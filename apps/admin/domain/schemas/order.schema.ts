...
const orderItemSchema = z.object({
  ...
  original_price_per_day: z.number().nonnegative().optional(),
...
export const CreateOrderSchema = z
  .object({
    ...
    rental_start_date: z
      .string()
      .datetime({ message: 'Invalid rental start date' })
      .or(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)),
    rental_end_date: z
      .string()
      .datetime({ message: 'Invalid rental end date' })
      .or(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)),
    pickup_time: z
      .string()
