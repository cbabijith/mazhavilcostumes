...
export default function CalendarPage() {
  ...
  const selectedSummary = selectedDate ? calendar.daySummaryMap.get(selectedDate) || null : null;
      ...
      <div className="h-[calc(100vh-3.5rem)] -m-8 p-8 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="bg-white border border-slate-200 shadow-sm rounded-lg p-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-2">Select a Branch</h3>
            <p className="text-sm text-slate-500 max-w-md mx-auto">
              Choose a branch from the top navigation to view its booking calendar. The calendar
              shows all active and scheduled orders for the selected branch.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] -m-8 p-8 overflow-hidden flex flex-col">
      ...
      <div className="shrink-0 mt-3">
        <CalendarStats stats={calendar.monthStats} isLoading={calendar.isLoading} />
      </div>

      {/* Main content: fills ALL remaining vertical space */}
      <div
        className={`flex-1 min-h-0 mt-3 grid gap-3 ${
          selectedDate ? 'grid-cols-1 lg:grid-cols-[1fr_340px]' : 'grid-cols-1'
        }`}
      >
        ...
        {selectedDate && (
          <div className="min-h-0 h-full overflow-hidden">
            <CalendarDayDetail summary={selectedSummary} onClose={handleCloseDetail} />
          </div>
        )}
      </div>
    </div>
