...
async function runTests() {
  ...
  console.log(
    `Using Environment: Branch ${branch.id.slice(0, 6)}, Category ${category.id.slice(0, 6)}`
  );
  ...
  const v1 = await productService.createProduct({
    slug: `slug-${ts}`,
  ...
  const v2 = await productService.createProduct({
    name: 'Test',
    category_id: category.id,
    store_id: store.id,
    price_per_day: 500,
  ...
  const v3 = await productService.createProduct({
    name: 'Test',
    slug: `slug2-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const f1 = await productService.createProduct({
    name: 'T',
    slug: `f1-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const f2 = await productService.createProduct({
    name: 'T',
    slug: `f2-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const f3 = await productService.createProduct({
    name: 'T',
    slug: `f3-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  assert(
    !f3.success || f3.data?.price_per_day === 999999999999,
    '6. Handles extreme large pricing gracefully (rejected or saved accurately)'
  );
  if (f3.success) await productService.deleteProduct(f3.data!.id);

  const f4 = await productService.createProduct({
    name: 'T',
    slug: `f4-${ts}`,
    category_id: category.id,
    store_id: store.id,
    branch_id: branch.id,
    price_per_day: 0.0000000001,
    quantity: 5,
  } as any);
  assert(
    !f4.success || (f4.data?.price_per_day ?? 0) > 0,
    '7. Handles micro-decimal pricing gracefully without crashing'
  );
  ...
  const i1 = await productService.createProduct({
    name: 'T',
    slug: `i1-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const i2 = await productService.createProduct({
    name: 'T',
    slug: `i2-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const i3 = await productService.createProduct({
    name: 'T',
    slug: `i3-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  assert(
    !i3.success || !!(i3.data && i3.data.available_quantity <= i3.data.quantity),
    '10. Mathematical Impossibility: available_quantity cannot exceed total quantity'
  );
  ...
  const z1 = await productService.createProduct({
    name: 'T',
    slug: `z1-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  assert(
    !z1.success || z1.data!.id !== '11111111-1111-1111-1111-111111111111',
    '11. ID injection attack via payload is ignored or rejected'
  );
  if (z1.success) await productService.deleteProduct(z1.data!.id);

  const z2 = await productService.createProduct({
    name: 'T',
    slug: `z2-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  assert(
    !z2.success || !z2.data!.created_at.includes('1990-01-01'),
    '12. created_at injection attack is ignored'
  );
  if (z2.success) await productService.deleteProduct(z2.data!.id);

  const z3 = await productService.createProduct({
    name: 'T',
    slug: `z3-${ts}`,
    category_id: category.id,
    store_id: store.id,
    branch_id: branch.id,
    price_per_day: '500' as any,
    quantity: 5,
  } as any);
  assert(
    !z3.success || typeof z3.data!.price_per_day === 'number',
    '13. Type spoofing (sending string for number) is rejected or cast correctly'
  );
  ...
  const e1 = await productService.createProduct({
    name: longStr,
    slug: `e1-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const e2 = await productService.createProduct({
    name: zalgo,
    slug: `e2-${ts}`,
    category_id: category.id,
    store_id: store.id,
  ...
  const e3 = await productService.createProduct({
    name: 'Emoji',
    slug: emojiSlug,
    category_id: category.id,
    store_id: store.id,
  ...
  assert(
    e3.success || e3.error?.code === 'VALIDATION_ERROR',
    '16. Emoji in slug is either stripped/allowed or properly caught by validation'
  );
  ...
  const h2 = await productService.createProduct({
    name: 'Collision',
    slug: baseSlug,
    category_id: category.id,
    store_id: store.id,
  ...
  assert(
    !h2.success && h2.error?.code === 'SLUG_EXISTS',
    '18. Slug collision constraint prevents duplicate slugs'
  );

  // --- PHASE 7: Category & Relational Edge Cases ---
  console.log('\n--- Phase 7: Category & Relational Edge Cases ---');

  const c1 = await productService.createProduct({
    name: 'T',
    slug: `c1-${ts}`,
  ...
  const c2 = await productService.createProduct({
    name: 'T',
    slug: `c2-${ts}`,
  ...
  assert(
    !c2.success,
    "20. Ghost category (valid UUID but doesn't exist) fails gracefully (foreign key violation wrapped)"
  );

  const c3 = await productService.updateProduct(createdProductId, {
    category_id: randomUuid,
  } as any);
  ...
  const j1 = await productService.updateProduct(createdProductId, {
    images: 'not-an-array' as any,
  });
  assert(!j1.success, '22. Sending string instead of array for JSONB images is blocked');

  const j2 = await productService.updateProduct(createdProductId, {
    images: [{ url: "<script>alert('xss')</script>", is_primary: true, sort_order: 1 }],
  });
  // It might allow it depending on Zod URL validation, but it shouldn't crash the server
  assert(
    j2.success || !j2.success,
    '23. XSS injection in JSONB array URL does not crash server (Caught by URL validator or saved safely)'
  );

  const massiveArray = Array(5000).fill({
  ...
  assert(
    !j3.success || j3.data!.images.length === 5000,
    '24. Massive JSONB array (5000 items) is either handled or rejected (DoS protection)'
  );
  ...
  assert(
    !z4.success || (dbCheckZ4.data as any)!.reserved_quantity !== undefined,
    '25. Injecting reserved_quantity update is ignored or strictly rejected'
  );
  ...
  assert(
    !z5.success || (dbCheckZ5.data as any)!.store_id === store.id,
    '26. Cross-tenant mutation attempt (changing store_id) is blocked'
  );
  ...
  assert(
    dbCheckConc.data!.price_per_day === 600 || dbCheckConc.data!.price_per_day === 700,
  ...
  if (customer) {
    ...
    assert(
      !o2.success || dbCheckInv.data!.quantity >= 5,
      '29. Decreasing total_quantity when items are actively rented is either blocked or accurately reflected'
    );

    // Attempt referential deletion
    const d1 = await productService.deleteProduct(createdProductId);
    assert(
      !d1.success && d1.error?.code === 'CANNOT_DELETE',
      '30. Cannot delete product with active orders (Referential Integrity)'
    );
  ...
  assert(
    !g1.success && g1.error?.code === 'NOT_FOUND',
    '32. Fetching non-existent product returns NOT_FOUND'
  );

  const d2 = await productService.deleteProduct(randomUuid);
  assert(
    !d2.success && d2.error?.code === 'NOT_FOUND',
    '33. Deleting non-existent product returns NOT_FOUND'
  );

  const u1 = await productService.updateProduct(randomUuid, { price_per_day: 100 });
  assert(
    !u1.success && u1.error?.code === 'NOT_FOUND',
    '34. Updating non-existent product returns NOT_FOUND'
  );
  ...
  assert(
    l2.success && l2.data!.products.length <= 100,
    '37. Fetching with limit: 1000000 caps at maximum safety threshold (e.g. 100)'
  );

  const l3 = await productService.getProducts({ page: 999999 } as any);
  assert(
    l3.success && l3.data!.products.length === 0,
    '38. Deep pagination beyond data bounds returns empty array safely'
  );
  ...
  if (s1.success) {
    ...
    assert(
      s2.success || (!s2.success && s2.error?.code === 'SLUG_EXISTS'),
      '39. Creating a product with a slug that was previously deleted is either cleanly allowed or correctly rejected'
    );
