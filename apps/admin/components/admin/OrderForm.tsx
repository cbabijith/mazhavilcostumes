...
export default function OrderForm({ initialData }: OrderFormProps) {
  ...
  const [startDate, setStartDate] = useState<Date>(
    initialData
      ? parseDateLocal(initialData.start_date || initialData.rental_start_date)
      : new Date()
  );
  const [endDate, setEndDate] = useState<Date>(
    initialData
      ? parseDateLocal(initialData.end_date || initialData.rental_end_date)
      : addDays(new Date(), 2)
  );
  ...
  const [orderDiscountType, setOrderDiscountType] = useState<'flat' | 'percent'>(
  ...
  const [advancePaymentMethod, setAdvancePaymentMethod] = useState<string>(
    initialData?.advance_payment_method || PaymentMethod.CASH
  );

  // Security Deposit State
  const [securityDeposit, setSecurityDeposit] = useState<number>(
    initialData?.security_deposit || 0
  );
  const [depositCollected, setDepositCollected] = useState<boolean>(
    initialData?.deposit_collected || false
  );
  const [depositPaymentMethod, setDepositPaymentMethod] = useState<string>(
    initialData?.deposit_payment_method || PaymentMethod.CASH
  );
  ...
  const addToCart = (product: any) => {
    ...
    const searchMaxAvail =
      (searchAvail as any)?.availableWithPriority ?? searchAvail?.available ?? 0;
    ...
    if (searchAvail && searchMaxAvail < 1) {
      ...
      showError(
        ...
        hasBuffer
          ? `0 available for the selected dates (even with priority cleaning).`
          : `0 available for the selected dates.`
      );
      return;
    }
    ...
    setCartItems((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          original_price_per_day: product.price_per_day,
          discount: 0,
  ...
  const handleBarcodeScan = async (barcode: string) => {
    ...
    try {
      ...
      showSuccess(
        wasExisting ? `${product.name} — quantity increased` : `${product.name} added to cart`
      );
    } catch {
    ...
    createCustomer(
      {
        name: quickAddName,
        phone: quickAddPhone,
        alt_phone: quickAddAltPhone || undefined,
        address: quickAddAddress || undefined,
      },
      {
        onSuccess: (res: any) => {
  ...
  const handleCheckout = async () => {
    ...
    const basePayload = {
      ...
      deposit_payment_method:
        securityDeposit > 0 && depositCollected ? depositPaymentMethod : undefined,
          ...
          onError: async (error: any) => {
            ...
            if (error?.message?.includes('PRIORITY_CLEANING_REQUIRED')) {
              showError(
                ...
                'Please confirm priority cleaning before placing this order.'
              );
            }
          },
        }
      ...
      <div className="space-y-6">
              ...
              {isEditing ? (
                <div className="space-y-0.5 mt-1 text-xs text-slate-500">
                  ...
                  {initialData.updated_at && initialData.updater && (
                    <p>
                      Last updated on{' '}
                      {format(new Date(initialData.updated_at), 'dd MMM, yyyy • h:mm a')} by{' '}
            ...
            <div className="bg-white border border-slate-200 rounded-lg p-5 space-y-4">
              ...
              {isBackdated && (
                <div className="flex items-center gap-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
                  ...
                  <span>
                    ⚠️ Backdated Bill: You are creating/editing an order with a pickup date in the
                    past.
                  </span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 p-2.5 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">
                ...
                <span className="flex-1">
                  Most products are blocked <strong>1 day before pickup</strong> and{' '}
                  <strong>1 day after return</strong> for preparation. (Note: Some categories like
                  Ornaments are exempt from this gap).
                </span>
              </div>
            </div>
                            ...
                            <div className="flex flex-col">
                              <ul className="py-1">
                                {availableProducts.map((p: any) => {
                                  ...
                                  const sMaxAvail =
                                    (sAvail as any)?.availableWithPriority ??
                                    sAvail?.available ??
                                    0;
                                  const branchInv = selectedBranchId
                                    ? p.product_inventory?.find(
                                        (inv: any) => inv.branch_id === selectedBranchId
                                      )
                                    : null;
                                  const pAvailableQty = selectedBranchId
                                    ? branchInv
                                      ? branchInv.available_quantity
                                      : 0
                                              ...
                                              : sAvail
                                                ? sAvail.available > 0
                                                  ...
                                                  : (() => {
                                                      ...
                                                      const cat = Array.isArray(cData)
                                                        ? cData[0]
                                                        : cData;
                                                      const hasBuffer = cat?.has_buffer ?? true;
                                                      return hasBuffer
                                                        ? `0 free (${sMaxAvail} with priority cleaning)`
                                                        : `0 free (Unavailable)`;
                                                    })()
                  ...
                  <ul className="space-y-2">
                    {cartItems.map((item) => {
                      ...
                      return (
                        <li
                          key={item.product.id}
                          className={`p-3 rounded-lg border bg-white ${isItemUnavailable ? 'border-red-200 bg-red-50/30' : 'border-slate-100'}`}
                        >
                          <div className="flex items-center gap-3">
                                ...
                                <input
                                  ...
                                  onBlur={(e) => {
                                    ...
                                    if (isNaN(val) || val < minPrice) {
                                      showError(
                                        'Price Override',
                                        `Price cannot be lower than ${formatCurrency(minPrice)}.`
                                      );
                                      setCartItems((prev) =>
                                        prev.map((p) =>
                                          p.product.id === item.product.id
                                            ? { ...p, price_per_day: minPrice }
                                            : p
                                        )
                                      );
                                    }
                          ...
                          {avail && (
                            <div
                              className={`mt-2 px-2 py-1.5 rounded-md text-xs flex items-center gap-1.5 ${isItemUnavailable ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}`}
                            >
                                ...
                                (() => {
                                  ...
                                  if (isItemUnavailable) {
                                    return (
                                      <>
                                        ...
                                        <span>
                                          Unavailable — only <strong>{avail.available}</strong> free
                                          for these dates (need {item.quantity})
                                        </span>
                                      </>
                                    );
                                  }
                                  ...
                                  return (
                                    <>
                                      ...
                                      <span>
                                        {avail.available} of {avail.total} free for this period
                                        {hasBuffer && (avail as any).priorityCleaningNeeded ? (
                                          <span className="text-amber-600 ml-1">
                                            ({(avail as any).availableWithPriority} with priority
                                            cleaning)
                                          </span>
                                        ) : null}
                                      </span>
                          ...
                          {avail &&
                            (() => {
                              ...
                              const actualConflicts = avail.overlappingOrders.filter(
                                (o: any) => !o.bufferOnly && isRelevant(o)
                              );
                              const nearbyBookings = avail.overlappingOrders.filter(
                                (o: any) => o.bufferOnly && isRelevant(o)
                              );
                                    ...
                                    (() => {
                                      ...
                                      return (
                                        <div className="mt-1 px-2 py-1.5 rounded bg-slate-50 border border-slate-100">
                                          ...
                                          <div className="text-[10px] text-slate-500 flex flex-col gap-0.5 mt-1 p-1.5 rounded bg-white/50 border border-slate-200/50">
                                            <div className="flex items-center justify-between">
                                              <span className="font-bold text-slate-800">
                                                ...
                                                {firstBooking.quantity}{' '}
                                                {firstBooking.quantity > 1 ? 'units' : 'unit'}
                                              </span>
                                              <div className="flex items-center gap-2">
                                                <span className="text-slate-600">
                                                  {fmtShort(firstBooking.startDate)} –{' '}
                                                  {fmtShort(firstBooking.endDate)}
                                                </span>
                                                <span
                                                  className={`${firstBooking.bufferSkipped ? 'text-slate-400 line-through decoration-amber-500/50' : 'text-amber-600 font-medium'} border-l border-slate-200 pl-2`}
                                                >
                                                  {firstBooking.bufferStartDate &&
                                                  firstBooking.bufferEndDate
                                                    ? `Gap: ${fmtShort(firstBooking.bufferStartDate)} & ${fmtShort(firstBooking.bufferEndDate)}`
                                                    : 'No Buffer'}
                                                  {firstBooking.bufferSkipped && (
                                                    <span className="text-[8px] ml-1 text-amber-600 font-bold">
                                                      (Prior Cleaning ON)
                                                    </span>
                                                  )}
                                                </span>
                                          ...
                                          {isExpanded &&
                                            allBookings.slice(1).map((o: any, idx: number) => (
                                              <div
                                                key={idx}
                                                className="text-[10px] text-slate-500 flex flex-col gap-0.5 mt-1 p-1.5 rounded bg-white/50 border border-slate-200/50 animate-in fade-in slide-in-from-top-1 duration-150"
                                              >
                                                <div className="flex items-center justify-between">
                                                  <span className="font-bold text-slate-800">
                                                    {o.customerName} — {o.quantity}{' '}
                                                    {o.quantity > 1 ? 'units' : 'unit'}
                                                  </span>
                                                  <div className="flex items-center gap-2">
                                                    <span className="text-slate-600">
                                                      {fmtShort(o.startDate)} –{' '}
                                                      {fmtShort(o.endDate)}
                                                    </span>
                                                    <span
                                                      className={`${o.bufferSkipped ? 'text-slate-400 line-through decoration-amber-500/50' : 'text-amber-600 font-medium'} border-l border-slate-200 pl-2`}
                                                    >
                                  ...
                                  {nearbyBookings.length > 0 && item.quantity > avail.available && (
                                    <div className="mt-1 px-2 py-1.5 rounded-md bg-amber-50/70 border border-amber-200/60">
                                      <p className="text-[10px] font-semibold text-amber-700 uppercase tracking-wider mb-0.5 flex items-center gap-1">
                                        <Info className="w-3 h-3" /> Nearby bookings{' '}
                                        <span className="normal-case font-normal text-amber-500">
                                          (within gap period — auto-allowed)
                                        </span>
                                      </p>
                                      <p className="text-[9px] text-amber-500 mt-1">
                                        Product will be returned before this rental starts. Ensure
                                        it&apos;s prepared in time.
                                      </p>
                                    </div>
                                  )}
                                </>
                          ...
                          {avail &&
                            ...
                            (() => {
                              ...
                              for (const info of allInfo) {
                                if (accumulated >= neededExtra) break;
                                const useFromThis = Math.min(
                                  info.returningQuantity,
                              ...
                              return (
                                <div className="mt-2 px-2.5 py-2 rounded-lg bg-amber-50 border border-amber-200">
                                  <div className="flex items-center gap-1.5 text-[11px] text-amber-800 font-medium">
                                    ...
                                    <span>
                                      <strong>{avail.available}</strong> freely available, need{' '}
                                      <strong>{neededExtra}</strong> via priority cleaning
                                    </span>
                                  </div>
                                  {relevantInfo.map((info: any, i: number) => (
                                    <div key={i} className="text-[10px] text-amber-700 mt-1 ml-5">
                                      {info.direction === 'returning_before' ? (
                                        <span>
                                          ⚡{' '}
                                          {info.usedQuantity === info.returningQuantity
                                            ? `${info.returningQuantity}`
                                            : `${info.usedQuantity} of ${info.returningQuantity}`}{' '}
                                          units — rush-clean from{' '}
                                          <strong>{info.returningOrderCustomer}</strong> (returning{' '}
                                          {new Date(info.returningOrderEndDate).toLocaleDateString(
                                            undefined,
                                        ...
                                        <span>
                                          ⚡ {info.usedQuantity}{' '}
                                          {info.usedQuantity === 1 ? 'unit' : 'units'} — skip prep
                                          for <strong>{info.returningOrderCustomer}</strong> (pickup{' '}
                                          {new Date(
                                            info.returningOrderStartDate
                          ...
                          <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50">
                            <div className="flex items-center border border-slate-200 rounded-md bg-white overflow-hidden">
                              <button
                                ...
                                className="px-2.5 py-1 hover:bg-slate-50 text-slate-500"
                              >
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const maxQty =
                                    (cartAvail as any)?.availableWithPriority ??
                                    cartAvail?.available ??
                                    0;
                            ...
                            <div className="text-right">
                              {(() => {
                                const lineTotal =
                                  item.price_per_day * item.quantity * pricingMultiplier;
                                const discAmt =
                                  item.discount_type === 'percent'
                                ...
                                return (
                                  <>
                                    ...
                                    <span className="text-sm font-bold text-slate-900">
                                      {formatCurrency(lineTotal - effectiveDisc)}
                                    </span>
                                  </>
                                );
                              })()}
                          ...
                          {isGstEnabled &&
                            (() => {
                              const gstInfo = cartTotals.itemGstBreakdown.find(
                                (g) => g.productId === item.product.id
                              );
                              ...
                              return (
                                <div className="flex items-center justify-between mt-1.5 px-1 py-1 bg-blue-50/50 rounded text-[10px]">
                                  ...
                                  <span className="text-blue-600 font-semibold">
                                    ₹{gstInfo.baseAmount.toFixed(2)} + ₹
                                    {gstInfo.gstAmount.toFixed(2)}
                                  </span>
                                </div>
                              );
                            })()}
              ...
              <div className="bg-slate-50 border-t border-slate-200 p-5 space-y-3">
                <div>
                  ...
                  <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1">
                    ...
                    {cartItems.length} {cartItems.length === 1 ? 'item' : 'items'} · {rentalDays}{' '}
                    days ({format(startDate, 'MMM d')} – {format(endDate, 'MMM d, yyyy')}) · Rate: ×
                    {pricingMultiplier}
                  </div>
                </div>
                {cartTotals.itemDiscountTotal > 0 && (
                  <div className="flex justify-between text-sm text-orange-600">
                    ...
                    <span className="font-medium">
                      −{formatCurrency(cartTotals.itemDiscountTotal)}
                    </span>
                  </div>
                )}
                {cartTotals.effectiveOrderDiscount > 0 && (
                  <div className="flex justify-between text-sm text-purple-600">
                    <span className="flex items-center gap-1">
                      <Percent className="w-3 h-3" /> Order Discount
                    </span>
                    <span className="font-medium">
                      −{formatCurrency(cartTotals.effectiveOrderDiscount)}
                    </span>
                  </div>
                )}
                ...
                <div className="flex justify-between items-end pt-3 border-t border-slate-200">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1">
                      <CalendarDays className="w-3 h-3" />
                      {rentalDays} days · {format(startDate, 'MMM d')} –{' '}
                      {format(endDate, 'MMM d, yyyy')} · Rate: ×{pricingMultiplier}
                ...
                <div className="border-t border-slate-200 p-5 space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500 font-semibold text-sm">
                        {orderDiscountType === 'percent' ? '%' : '₹'}
                      </span>
                      <Input
                        type="number"
                    ...
                    <div className="space-y-2">
                      ...
                      {advanceExceedsTotal && (
                        <p className="text-xs text-red-600 font-medium flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3" /> Advance cannot exceed grand total (
                          {formatCurrency(cartTotals.grandTotal)})
                        </p>
                      )}
                      {isFullAdvancePayment && (
                        <div className="px-3 py-2 bg-green-50 border border-green-200 rounded-lg text-xs text-green-700 font-medium flex items-center gap-1.5">
                          <CheckCircle2 className="w-3.5 h-3.5" /> Full payment collected — no
                          balance due at return
                        </div>
                      )}
                      {!advanceExceedsTotal && !isFullAdvancePayment && advanceAmount > 0 && (
                        <div className="px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700 flex items-center gap-1.5">
                          ...
                          Balance due at return:{' '}
                          {formatCurrency(cartTotals.grandTotal - advanceAmount)}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="border-t border-slate-200 p-5 space-y-3">
                {advanceAmount > 0 && !advanceExceedsTotal && (
                  <>
                    ...
                    <div className="flex justify-between text-sm font-bold text-slate-900">
                      ...
                      <span>
                        {formatCurrency(Math.max(0, cartTotals.grandTotal - advanceAmount))}
                      </span>
                    </div>
                  </>
                )}
                ...
                {hasUnavailableItems && (
                  <div className="flex items-center gap-2 p-2.5 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
                    ...
                    <span>
                      Some items are <strong>not available</strong> for the selected dates. Adjust
                      quantities or change dates.
                    </span>
                  </div>
                )}
                {isPaymentInvalid && (
        ...
        {isQuickAddMode && (
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget && !isCreatingCustomer) setIsQuickAddMode(false);
            }}
          >
                ...
                <div className="space-y-1.5">
                  ...
                  {quickAddPhone.length > 0 && quickAddPhone.length !== 10 && (
                    <p className="text-xs text-red-500">
                      Phone number must be 10 digits ({quickAddPhone.length}/10)
                    </p>
                  )}
                </div>
                <div className="space-y-1.5">
                  ...
                  {quickAddAltPhone.length > 0 && quickAddAltPhone.length !== 10 && (
                    <p className="text-xs text-red-500">
                      Phone number must be 10 digits ({quickAddAltPhone.length}/10)
                    </p>
                  )}
                </div>
                <div className="space-y-1.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md mx-4 p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-slate-600 mb-4">
              The following products need priority cleaning to fulfill this order. Staff will be
              notified in the Cleaning Queue.
            </p>
            <div className="space-y-2 mb-5 max-h-48 overflow-y-auto">
              {priorityCleaningItems.map((info: any, i: number) => (
                <div
                  key={i}
                  className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-amber-50 border border-amber-200"
                >
                      ...
                      {info.direction === 'returning_before' ? (
                        <>
                          {info.usedQuantity === info.returningQuantity
                            ? info.returningQuantity
                            : `${info.usedQuantity} of ${info.returningQuantity}`}{' '}
                          units — rush-clean from <strong>{info.returningOrderCustomer}</strong>{' '}
                          (returning{' '}
                          {new Date(info.returningOrderEndDate).toLocaleDateString(undefined, {
                            month: 'short',
                            day: 'numeric',
                          })}
                          )
                        ...
                        <>
                          {info.usedQuantity} {info.usedQuantity === 1 ? 'unit' : 'units'} — skip
                          prep for <strong>{info.returningOrderCustomer}</strong> (pickup{' '}
                          {new Date(info.returningOrderStartDate).toLocaleDateString(undefined, {
                            month: 'short',
                            day: 'numeric',
                          })}
                          )
