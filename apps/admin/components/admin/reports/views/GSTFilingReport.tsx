        ...
        <div className="flex items-start gap-4 p-5 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-600 mt-1 leading-relaxed">
              GST is turned off in your store settings. Orders created while GST is disabled will
              not have any tax data, so they won&apos;t appear in this report. To start collecting
              GST on new orders, enable it from Settings.
            </p>
          </div>
          <Link
          ...
          <Card className="shadow-sm border-slate-200 bg-white">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-black text-slate-900">
                {formatCurrency(reportSummary.total_taxable)}
              </p>
            </CardContent>
          </Card>
          <Card className="shadow-sm border-slate-200 bg-white">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-black text-slate-900">
                {formatCurrency(reportSummary.total_cgst)}
              </p>
            </CardContent>
          </Card>
          <Card className="shadow-sm border-slate-200 bg-white">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-black text-slate-900">
                {formatCurrency(reportSummary.total_sgst)}
              </p>
            </CardContent>
          </Card>
          <Card className="shadow-sm border-slate-200 bg-white">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-black text-emerald-600">
                {formatCurrency(reportSummary.total_gst)}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        footer={
          reportSummary && (
            <tfoot className="bg-slate-50 font-bold text-slate-900">
              <tr>
                <td className="px-5 py-3 border-t border-slate-200 uppercase text-xs">
                  Total Liability
                </td>
                <td className="px-5 py-3 border-t border-slate-200">
                  {formatCurrency(reportSummary.total_taxable)}
                </td>
                <td className="px-5 py-3 border-t border-slate-200">
                  {formatCurrency(reportSummary.total_cgst)}
      ...
      {gstDetails.length > 0 && (
        <div className="mt-8 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-1">
            ...
            <Badge
              variant="secondary"
              className="bg-slate-100 text-slate-600 border-none px-3 py-1 font-semibold text-xs"
            >
