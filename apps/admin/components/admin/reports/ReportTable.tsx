  ...
  return (
    <Card className="shadow-sm border-slate-200 bg-white">
      <CardHeader className="border-b border-slate-200 py-3 px-6 flex flex-row items-center justify-between">
        ...
        <div className="text-xs text-slate-400 font-medium">Showing {data.length} records</div>
      </CardHeader>
        ...
        <table className="w-full text-sm text-left">
                ...
                >
                  <div className="flex items-center gap-1">
                    ...
                    {sortConfig?.key === col.key ? (
                      <TrendingUp
                        className={cn(
                          'w-3 h-3 transition-transform text-slate-900',
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr>
                <td colSpan={columns.length} className="px-5 py-12 text-center text-red-600">
                  {error}
                </td>
              </tr>
            ) : data.length === 0 ? (
