...
export default function ReviewPopup() {
  ...
  useEffect(() => {
    ...
    const reviewStatus = localStorage.getItem("parisbridals_review_status");
  ...
  const handleReviewClick = () => {
    localStorage.setItem("parisbridals_review_status", "reviewed");
    setIsVisible(false);
    window.open(
      "https://www.google.com/maps/search/?api=1&query=9.18014716987313,76.5576453469383",
      "_blank",
      "noopener,noreferrer"
    );
  };
  ...
  const handleDismiss = () => {
    localStorage.setItem("parisbridals_review_status", "dismissed");
    setIsVisible(false);
  };
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="mt-1">
            <h3 className="text-base font-bold text-heading font-serif leading-tight">
              Enjoying Paris Bridals?
            </h3>
            <p className="text-xs text-body font-light leading-relaxed mt-1.5">
              Help other brides discover our collections by sharing your experience on Google Maps. It only takes a minute!
            </p>
          </div>
