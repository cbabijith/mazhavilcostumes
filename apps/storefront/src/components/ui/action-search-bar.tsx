    ...
    <div className="relative" ref={dropdownRef}>
      <form onSubmit={handleSearch} className="relative">
        ...
        <div className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2">
          {query ? (
            <Send
              className="w-4 h-4 md:w-5 md:h-5 text-rosegold animate-in zoom-in duration-150"
              strokeWidth={1.5}
            />
          ) : (
            <Search
              className="w-4 h-4 md:w-5 md:h-5 text-muted-foreground animate-in zoom-in duration-150"
              strokeWidth={1.5}
            />
          )}
        </div>
        ...
        >
          <div className="space-y-4">
            ...
            {suggestions.length > 0 && (
              <div>
                <div className="text-[10px] uppercase tracking-widest font-bold text-caption mb-2 ml-2">
                  Suggested Products
                </div>
                <div className="space-y-1">
                  {suggestions.map((product) => (
                    ...
                    >
                      <div className="flex items-center gap-2 md:gap-3 min-w-0">
                        <Package
                          size={14}
                          className="text-muted-foreground group-hover:text-rosegold shrink-0"
                        />
                      ...
                      {product.category_name && (
                        <span className="text-[10px] text-caption uppercase tracking-wider shrink-0 ml-2 hidden sm:inline">
                          {product.category_name}
                        </span>
                      )}
                    </button>
                  ...
                  {isFetchingMore && (
                    <div className="py-2 text-center text-xs text-caption animate-pulse">
            ...
            {filteredActions.length > 0 && (
              <div>
                {suggestions.length > 0 && (
                  <div className="border-t border-[var(--border-silk)] my-3" />
                )}
                <div className="text-[10px] uppercase tracking-widest font-bold text-caption mb-2 ml-2">
                  Quick Actions
                </div>
                <div className="space-y-1">
                  {filteredActions.map((action) => (
                    ...
                    >
                      <div
                        className={cn(
                          'p-1.5 rounded-lg bg-silk-dark/30 group-hover:bg-white transition-colors',
                          action.color
                        )}
                      >
                        {action.icon}
            ...
            {!isLoading && suggestions.length === 0 && filteredActions.length === 0 && query && (
              <div className="p-4 text-center text-sm text-caption italic">
