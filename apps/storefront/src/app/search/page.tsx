  ...
  return (
    <main className="min-h-screen bg-silk">
      ...
      <div className="lg:hidden">
        <SearchClient storeId={store.id} categories={categories} featured={featured} />
      </div>
      ...
      <div className="hidden lg:block max-w-7xl mx-auto px-8 py-20 text-center">
        ...
        <p className="text-body max-w-md mx-auto mb-10">
          Use the search bar in the header to find specific pieces or browse our categories below.
        </p>
        <div className="grid grid-cols-4 gap-6">
          {categories.slice(0, 4).map((cat) => (
            <a
              key={cat.id}
              href={`/collections?category_id=${cat.id}`}
              className="bg-white p-6 rounded-3xl shadow-silk hover:-translate-y-1 transition-all"
            >
